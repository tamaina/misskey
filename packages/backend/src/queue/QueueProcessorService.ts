...
export class QueueProcessorService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.systemQueueWorker
			...
			.on('failed', (job, err) => systemLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { e: renderError(err), data: job?.data }))
		...
		this.dbQueueWorker
			...
			.on('failed', (job, err) => dbLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { e: renderError(err), job }))
		...
		this.deliverQueueWorker
			...
			.on('failed', (job, err) => deliverLogger.warn(`failed(${err.message}) ${getJobInfo(job)} id=${job ? job.id : '-'} to=${job?.data.to}`, { e: renderError(err), data: job?.data }))
		...
		this.webhookDeliverQueueWorker
			...
			.on('failed', (job, err) => webhookLogger.warn(`failed(${err.message}) ${getJobInfo(job)} to=${job ? job.data.to : '-'}`, { e: renderError(err) }))
		...
		this.relationshipQueueWorker
			...
			.on('failed', (job, err) => relationshipLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { e: renderError(err), job }))
		...
		this.objectStorageQueueWorker
			...
			.on('failed', (job, err) => objectStorageLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { e: renderError(err), job }))
