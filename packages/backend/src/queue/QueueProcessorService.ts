...
export class QueueProcessorService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.dbQueueWorker
			...
			.on('failed', (job, err) => dbLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { job, e: renderError(err) }))
		...
		this.deliverQueueWorker
			...
			.on('failed', (job, err) => deliverLogger.warn(`failed(${err.message}) ${getJobInfo(job)} id=${job ? job.id : '-'} to=${job?.data.to}`, { data: job?.data, e: renderError(err) }))
		...
		this.webhookDeliverQueueWorker
			...
			.on('failed', (job, err) => webhookLogger.warn(`failed(${err.message}) ${getJobInfo(job)} to=${job ? job.data.to : '-'}`, { e: renderError(e) }))
		...
		this.relationshipQueueWorker
			...
			.on('failed', (job, err) => relationshipLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { job, e: renderError(err) }))
		...
		this.objectStorageQueueWorker
			...
			.on('failed', (job, err) => objectStorageLogger.warn(`failed(${err.message}) id=${job ? job.id : '-'}`, { job, e: renderError(err) }))
