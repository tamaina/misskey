		...
		<div v-for="achievement in achievements" :key="achievement.name" :class="$style.achievement" class="_panel">
			...
			<div :class="$style.body">
				<div :class="$style.header">
					<span :class="$style.title">{{ $locale.env._achievements._types[`_${achievement.name}`].title }}</span>
				...
				<div :class="$style.description">{{ withDescription ? $locale.env._achievements._types[`_${achievement.name}`].description : '???' }}</div>
				<div v-if="'flavor' in $locale.env._achievements._types[`_${achievement.name}`] && withDescription" :class="$style.flavor">{{ ($locale.env._achievements._types[`_${achievement.name}`] as { flavor: string; }).flavor }}</div>
			</div>
		</div>
