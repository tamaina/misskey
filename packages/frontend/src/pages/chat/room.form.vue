...
>
	<textarea
		...
		class="_acrylic"
		:placeholder="$locale.env.inputMessageHere"
	...
	<footer :class="$style.footer">
		...
		<div :class="$style.buttons">
			...
			<button class="_button" :class="[$style.button, $style.send]" :disabled="!canSend || sending" :title="$locale.env.send" @click="send">
...
async function onPaste(ev: ClipboardEvent) {
	...
	if (items.length === 1) {
		...
		if (items[0].kind === 'file') {
			os.alert({
				type: 'error',
				text: localeRef.value.env.onlyOneFileCanBeAttached,
			});
		}
	}
}
