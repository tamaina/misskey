...
<SearchMarker markerId="botProtection" :keywords="['bot', 'protection', 'captcha', 'hcaptcha', 'mcaptcha', 'recaptcha', 'turnstile']">
	<MkFolder>
		...
		<template #label><SearchLabel>{{ $locale.env.botProtection }}</SearchLabel></template>
		...
		<template v-else #suffix>{{ $locale.env.none }} ({{ $locale.env.notRecommended }})</template>
		...
		<div class="_gaps_m">
			<MkRadios
				...
				:options="[
					{ value: 'none', label: `${$locale.env.none} (${$locale.env.notRecommended})` },
			...
			<template v-if="botProtectionForm.state.provider === 'hcaptcha'">
				<MkInput v-model="botProtectionForm.state.hcaptchaSiteKey" debounce>
					...
					<template #label>{{ $locale.env.hcaptchaSiteKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.hcaptchaSecretKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.hcaptchaSecretKey }}</template>
				</MkInput>
				<FormSlot v-if="botProtectionForm.state.hcaptchaSiteKey">
					<template #label>{{ $locale.env._captcha.verify }}</template>
					<MkCaptcha
				...
				<MkInfo>
					<div :class="$style.captchaInfoMsg">
						<div>{{ $locale.env._captcha.testSiteKeyMessage }}</div>
						<div>
			...
			<template v-else-if="botProtectionForm.state.provider === 'mcaptcha'">
				<MkInput v-model="botProtectionForm.state.mcaptchaSiteKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.mcaptchaSiteKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.mcaptchaSecretKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.mcaptchaSecretKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.mcaptchaInstanceUrl" debounce>
					<template #prefix><i class="ti ti-link"></i></template>
					<template #label>{{ $locale.env.mcaptchaInstanceUrl }}</template>
				</MkInput>
			...
			<template v-else-if="botProtectionForm.state.provider === 'recaptcha'">
				<MkInput v-model="botProtectionForm.state.recaptchaSiteKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.recaptchaSiteKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.recaptchaSecretKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.recaptchaSecretKey }}</template>
				</MkInput>
			...
			<template v-else-if="botProtectionForm.state.provider === 'turnstile'">
				<MkInput v-model="botProtectionForm.state.turnstileSiteKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.turnstileSiteKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.turnstileSecretKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ $locale.env.turnstileSecretKey }}</template>
				</MkInput>
				...
				<MkInfo>
					<div :class="$style.captchaInfoMsg">
						<div>
							{{ $locale.env._captcha.testSiteKeyMessage }}
						</div>
						<div>
			...
			<template v-else-if="botProtectionForm.state.provider === 'testcaptcha'">
				<MkInfo warn><span v-html="$locale.env.testCaptchaWarning"></span></MkInfo>
				<FormSlot>
					<template #label>{{ $locale.env._captcha.verify }}</template>
...
const errorHandler: ApiWithDialogCustomErrors = {
	...
	'0f4fe2f1-2c15-4d6e-b714-efbfcde231cd': {
		title: localeRef.value.env._captcha._error._requestFailed.title,
		text: localeRef.value.env._captcha._error._requestFailed.text,
	},
	// 検証リクエストの結果が不正
	'c41c067f-24f3-4150-84b2-b5a3ae8c2214': {
		title: localeRef.value.env._captcha._error._verificationFailed.title,
		text: localeRef.value.env._captcha._error._verificationFailed.text,
	},
	// 不明なエラー
	'f868d509-e257-42a9-99c1-42614b031a97': {
		title: localeRef.value.env._captcha._error._unknown.title,
		text: localeRef.value.env._captcha._error._unknown.text,
	},
};
