...
<XColumn :column="column" :isStacked="isStacked" :menu="menu" :refresher="async () => { await notificationsComponent?.reload() }">
	<template #header><i class="ti ti-bell" style="margin-right: 8px;"></i>{{ column.name || $locale.env._deck._columns.notifications }}</template>
...
const menu = [{
	icon: 'ti ti-pencil',
	text: localeRef.value.env.notificationSetting,
	action: func,
}];
</script>
