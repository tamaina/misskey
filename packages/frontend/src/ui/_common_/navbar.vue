...
<div :class="[$style.root, { [$style.iconOnly]: iconOnly }]">
	<div :class="$style.body">
		<div :class="$style.top">
			<button v-tooltip.noDelay.right="instance.name ?? $locale.env.instance" class="_button" :class="$style.instance" @click="openInstanceMenu">
				<img :src="instance.iconUrl || '/favicon.ico'" alt="" :class="$style.instanceIcon" style="view-transition-name: navbar-serverIcon;"/>
			</button>
			<button v-if="!iconOnly" v-tooltip.noDelay.right="$locale.env.realtimeMode" class="_button" :class="[$style.realtimeMode, store.r.realtimeMode.value ? $style.on : null]" @click="toggleRealtimeMode">
				<i v-if="store.r.realtimeMode.value" class="ti ti-bolt ti-fw"></i>
				<i v-else class="ti ti-bolt-off ti-fw"></i>
			</button>
			<button v-if="!iconOnly && showWidgetButton" v-tooltip.noDelay.right="$locale.env.widgets" class="_button" :class="[$style.widget]" @click="() => emit('widgetButtonClick')">
				<i class="ti ti-apps ti-fw"></i>
			</button>
		</div>
		<div :class="$style.middle">
			<MkA v-tooltip.noDelay.right="$locale.env.timeline" :class="$style.item" :activeClass="$style.active" to="/" exact>
				<i :class="$style.itemIcon" class="ti ti-home ti-fw" style="view-transition-name: navbar-homeIcon;"></i><span :class="$style.itemText">{{ $locale.env.timeline }}</span>
			</MkA>
			...
			<MkA v-if="$i != null && ($i.isAdmin || $i.isModerator)" v-tooltip.noDelay.right="$locale.env.controlPanel" :class="$style.item" :activeClass="$style.active" to="/admin">
				<i :class="$style.itemIcon" class="ti ti-dashboard ti-fw" style="view-transition-name: navbar-controlPanel;"></i><span :class="$style.itemText">{{ $locale.env.controlPanel }}</span>
			</MkA>
			<button class="_button" :class="$style.item" @click="more">
				<i :class="$style.itemIcon" class="ti ti-grid-dots ti-fw" style="view-transition-name: navbar-more;"></i><span :class="$style.itemText">{{ $locale.env.more }}</span>
				<span v-if="otherMenuItemIndicated" :class="$style.itemIndicator" class="_blink"><i class="_indicatorCircle"></i></span>
			</button>
			<MkA v-tooltip.noDelay.right="$locale.env.settings" :class="$style.item" :activeClass="$style.active" to="/settings">
				<i :class="$style.itemIcon" class="ti ti-settings ti-fw" style="view-transition-name: navbar-settings;"></i><span :class="$style.itemText">{{ $locale.env.settings }}</span>
			</MkA>
		</div>
		<div :class="$style.bottom">
			...
			<button v-tooltip.noDelay.right="$locale.env.note" class="_button" :class="[$style.post]" data-cy-open-post-form @click="() => { os.post(); }">
				<i class="ti ti-pencil ti-fw" :class="$style.postIcon"></i><span :class="$style.postText">{{ $locale.env.note }}</span>
			</button>
			<button v-if="$i != null" v-tooltip.noDelay.right="`${$locale.env.account}: @${$i.username}`" class="_button" :class="[$style.account]" @click="openAccountMenu">
...
function toggleRealtimeMode(ev: PointerEvent) {
	os.popupMenu([{
		type: 'label',
		text: localeRef.value.env.realtimeMode,
	}, {
		text: store.s.realtimeMode ? localeRef.value.env.turnItOff : localeRef.value.env.turnItOn,
